...
knitr::kable(cbind(
  ...
  c(
    'Department code',
    'Date of the tests. Format yyyy-mm-dd-yyyy-mm-dd ',
    ...
    'Population',
    'Daily patients testing positive',
    'Daily patients tested',
    'Incidence rate',
    'Positivity rate',
    'Screening rate'
  )
...
**Loading of the data** 
All but the single_parent dataset are CSV files with semicolons as separators. The single_parent data set is in excel format, so we have to use read_excel. Then, we use skip because the document includes extra header information rows.
...
```
5.  We want to know the attribution rate of each mention. It is needed in order to do some true comparisons between region without the same number of students.
```{r, echo = TRUE}
dnb_results <- dnb_results %>% 
...
```{r}
dnb_results <- dnb_results %>% 
dplyr::filter(!department_fr %in% c("Polynesie-Francaise","Guyane", "Martinique", "Guadeloupe", "La-Reunion", "Mayotte", "Saint-Martin", "NA", "-"))
```
6. The other data sets go down to the department level. We therefore have to create a simplified version for further merges between data sets. We will summarize all values to the department level.
```{r}
...
```
5.  We create a `high_school_type variable` which contains the information of agricultural_high_school, military_high_school, vocational_high_school.
```{r, echo = TRUE}
...
```
5.  We are not interested by students for higher education and therefore can delete their variables. 
```{r}
student_housing <- student_housing %>% 
...
```
7.  The other data sets go down to the department level. We therefore have to create a simplified version for further merges between data sets. We will summarize all values to the department level.
```{r}
