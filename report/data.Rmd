...
We have to rename all data sets in the same way. Therefore, we created a function. The function has a data frame and a vector as inputs. It checks if the length of the vector is correct, if so it returns a tibble with the column names renamed. Otherwise, the function gives an error message statting that the vector is not the right length. 
```{r, echo = TRUE}
...
```
5.  We want some to know the attribution rate of each mention to simplify the comparison.
```{r, echo = TRUE}
...
```
6.  we can see on the map below, we have establishment from the overseas collectivities (COM) as well French international school. We decided to remove all these establishment as their reality is very different from that experienced by French metropolitan students.
...
```{r}
establishment_24_fr <- establishment_24 %>% 
  select(-c(postcode:mail,city_school,QPV:SEGPA,establishment_web:ministry, precision_on_localisation)) %>% 
  dplyr::filter(!academy %in% c("Polynésie Française","Guyane", "Martinique", "Guadeloupe", "La Réunion", "Mayotte", "AEFE"))
```
...
map <- map_data("france")
```
The region variable is in fact the departments. We rename it to fit what we added in the other data sets.
```{r, echo = TRUE}
