...
### National analysis
For this first visualisation we wanted to see the evolution of the DNB results over time in order to have an overview of the data we were working with. For this, we created a data set summarizing the results at the national level. We simplified our visualisation process by pivoting the table. During the process, we created four new variables `Candidates`, `Number_of_students`, `Mention_type` and `Rate`. 
...
The number of students that were admitted without a mention has been decreasing since 2006. It reached 284'774 students (53.85%) in 2006, compared to 154'095 students (25.11%) in 2021. The negative tendency calmed itself during the 2017-2021 period. As it is the case for all mentions, there is a complete shift in 2017. This will be analysed in further detail in the part one of our analysis. Contrary to the students graduating without honours, the number of students admitted with the TB distinction is showing a positive pattern. It grew from 31'563 (4.49%) in 2006 to reach 204'523 (23.98%) in 2021. The pattern for the two other distinctions are relatively similar as they remained stable up until 2017 but the change was not as drastic as for the first two distinction mentioned. The number of students that are admitted with a good mention show a positive pattern. In fact, the number has developed from 91'719 students (13.66%) in 2006 to 180'774 students (25.07%) in 2021. The number of students admitted with an AB distinction is increased slightly from 2006 to 2021. It developed from 164'180 (28%) students in 2006 to 165'350 students (25.83%) in 2021, representing a growth rate of 0.71%.


### Regional Analysis

We started with a comparative analysis in order to compare the success rate between the regions. For this, we needed to create the charts below based on the dnb-results data set. 
...
##### Success rate {-}
*It is to be noted that for a better visual interpretation the scale of the y-axis is adapted for each graph.*
...
Following our national and regional analysis, we wanted to reduce the scope of our analysis further in order to have a better understanding of our data.
...
Moreover, we can see that the distribution remains stable over time for all ditinctions apart for the distinction TB. Indeed, up until 2017, the variance was very small with Q1 and Q3 being distant of around 2 percentage points. From 2017 onwards, it increased to nearly 5 percentage points.
...
After analyzing the data provided by the dnb-result data set and understanding the evolution of French secondary students’ performance during the years [2006-2021], as well as the most significant events that marked this period. It is time to explore the Establishment_24 data set.

We started our analysis by mapping the establishments labelled Generation 2024 across France using the ggplot’s functions, geo_polygon combined with geom_point. This helped us visualize the geographic distribution of the labelled establishments and understand where they are mostly located.
```{r, echo = TRUE, warning=FALSE, out.width='100%', out.height= '50%'}
...
ggplotly(p, tooltip = c("text", "text2"))
```
Based on the map, we conclude that the establishments labelled generation 2024 are mostly located in the region of Île-de-France, Auvergne-Rhône-Alpes, Occitanie and Nouvelle Aquitaine.However, we can hardly conclude anything with such graph and data. We decided to use the summary of labelled establishment by department for the rest of this exploration. 

####  New establishment 2024 labellization per department and year {.tabset .unnumbered}

After having a general idea about the geographic distribution of generation 2024’s establishments, we created this second map to analyze their evolution in the period [2017-2022], for each department. For this, we used the function filter and ggplot to map for each year, starting from 2017, the number of new labelled establishment, and this for all French departments.
```{r, echo=TRUE}
...
p <- establishment_24_dep_map  %>% 
  ...
  labs(x = "", 
       y = "", 
       title = " New establishment labelled Generation 2024 in 2017")+
  map_theme
...
p <-establishment_24_dep_map  %>% 
  ...
  labs(x = "", 
       y = "", 
       title = " New establishment labelled Generation 2024 in 2018")+
  map_theme
...
p <-establishment_24_dep_map  %>% 
  ...
  labs(x = "", 
       y = "", 
       title = "New establishment labelled Generation 2024 in 2019")+
  map_theme
...
p <-establishment_24_dep_map  %>% 
  ...
  labs(x = "", 
       y = "", 
       title = "New establishment labelled Generation 2024 in 2020")+
  map_theme
...
p <-establishment_24_dep_map  %>% 
  ...
  labs(x = "", 
       y = "", 
       title = "New establishment labelled Generation 2024 in 2021")+
  map_theme
...
p <-establishment_24_dep_map  %>% 
  ...
  labs(x = "", 
       y = "", 
       title = "New establishment labelled Generation 2024 in 2022")+
  map_theme
...
At first glance, we notice the positive evolution of the number of labelled establishments. In 2017, approximatively, twenty labeled establishments were recorded within the French departments, with four registered by the Nord, being the highest number reported that year. This tendency continued until 2020, when almost all departments recorded new labeled establishments.

However, this pattern was disrupted in 2021, when only 15 departments recorded an evolution of the number of their labeled establishments. Surprisingly, 2022 has been the peak year so far: Only four departments did not declare an increase in this number.

At this point of our analysis, we wanted to analyze the percentage of labeled establishments across all French departments. To do that, we computed the rate of 2024 establishments for each department. We first counted the number of establishment by department using the establishments present in the data set dnb_results. From this, we just had to measure the rate of labelled establishment. Next, we used the ggplot function to map our data.
```{r, echo=TRUE}
...
```
Based on the map, the department of Hautes-Pyrenees has the highest number of labeled establishments so far, with a rate of 52.78% in 2022. Haute-Corse took the second position, having 50% of its establishments labelled by the end of 2022. On the third position, 43.14% of the establishmentsof of Val-Doise were labeled by the end of 2022. Other departments have also shown interesting rates such as Essonne (41.43%), Corse du Sud (40%), Tarn-et-Garonne (40%).

It is interesting to see that even if the Olympic games will take place in Paris the establishments from the Ile de france region have relatively low labeling rates. Indeed apart from Val-Doise and Essone which have high labeling rates, the other departments are rather around 20%. The explanation could be related to the number of establishments in these departments. For example, Paris has 216 establishments whereas Val-Doise and Essone have respectively 153 and 140 establishments. Similarly the two Corsican departments and Hautes-Pyrenees only have 15, 16 and 36 establishments. To conclude it explains why some departments may have higher rates but based on the principle that establishments of a same department have a smimilar number of students and that we will compare this data to success rate by department it should not affect our end results.  


## student_housing

After analyzing French secondary students’ success rate on a national, regional and a departmental level, and exploring the establishment_24 dataset, it is time to focus on the students housing dataset. Our goal is to analyze the evolution of the different housing modes between 2020 and 2022. 

The following graph plots the number of students and groups them by their Housing option.
```{r, echo=TRUE, warning=FALSE, out.width='100%', out.height= '50%'}
p <- student_housing %>% 
...
Based on the chart, half boarding students represents the largest category, recording 3'654'401 students in 2020, compared to 1'536'776 external students, and only 191'100 boarding students in the same year. This category remains the undisputed leader in 2021 and 2022. The students’ distribution has not changed remarkably in the period [2020-2022]. However, we notice a slight decrease in the number of half-boarders ‘students. French secondary students are less and less interested in the half-boarding housing mode. They were 3'654'401 students in 2020, compared to 3'593'806 in 2022, representing a negative variation of 1.65%. Conversely, the external housing mode is attracting more and more students. The number of externals showed a positive change of 4.94% in 2022 compared to 2020. It increased from 1'536'776 students in 2019 to 1'612'770 in 2022.


####  Housing option rate in secondary education per department {.tabset .unnumbered}

To conduct a departmental analysis, we created the following graph, in which we plotted for each of the housing modes, the rate of students recorded in each specific category for all French departments.
...
**Half boarding housing mode** attracts the higher number of students in all French departments. On average, 69 % of students opted for this residence mode in 2020, 69.16% in 2021 and 68.88% in 2022. In 2020, 83.43% of Cotes_Darmor’ students were half boarders, which is the highest rate recorded in all French departments. Contrarily, only 49.7% of Seine-Saint-Denis’ students chose this housing mode, which is the lowest rate recorded that year. In 2021 and 2022, we notice the same distribution of half boarders’ students among French departments, with the highest rate registered by Cotes_Darmor and Vendee, and the lowest recorded with Seine-Saint-Denis. **External housing mode** is the second common one after the half boarding mode. In 2020, external students represented 24.71% of the total number of students, compared to 24.61% and 24.70% in 2021 and 2022, respectively. Landes, Cotes_Darmor and Vendee are the departments with the least number of external students, with a respective rate of 8.01 %, 8.42% and 8.49%. Seine-Saint-Denis occupies the first position, recording the highest number of externals: 50%. Similarly, in 2021 and 2022, those three departments recorded the lowest rates while the department of Seine-Saint-Denis recorded the highest. Finally, the **boarding housing mode** attracts the least number of students. It recorded the lowest rates over our study period. On average, boarding students represented 5.69% of the total number of students in 2020, 5.35 % in 2021 and 5.36 % in 2022. In 2020, Paris and Seine-Saint-Denis recorded the lowest rate of boarding students, 0.260% and 0.287 % respectively. Lozere in the other hand, recorded 25.79%, which is the highest rate of all the departments. We notice the same tendency in 2021 and 2022.


## single_parent
Our last data set related to socioeconomical factor is the single parent one, The first visualisation we wanted to do was a barplot that would show the evolution of the total number of single-parent families in France from 2007 to 2008. To do this, we had to isolate, the `sing_par` and `session` variables, then summarized the number of single-parents with “sum”. We then were able to use geom_bar.
...
We needed to create a data set to create maps.For this, we joined the single_parent and map data sets using `department_fr`.  
...
p <-sing_dnb_map %>% 
  ...
  labs(x = "", 
       y = "", 
       title = "Single parent families per student in 2007")+
  map_theme
...
First, we wanted to get an overview of COVID positive cases in France over the covid period. We selected from covid_in_school the variables `positive` and `test_date`, group by `test_date` and summarised `positive` with sum. We then use ggplot with `test_date` as the x-axis and `positive` for the y-axis. 
...
ggplotly(p, tooltip = c("x","y"))
```
From 2020 until the end of 2021, we notice waves of covid cases. Indeed, every 6 months, the number of positive cases increases to nearly 20'000 positive cases per day and then drops to around 1'000 positive cases per day. However, we observe a clear increase, with a peak of more than 235,000 positive cases per day, in January 2022. After this increase, the number of positive cases fell significantly to remain around 2'000-10'000 positive cases per day. The 2022 academic year is therefore a year to keep an eye on, for further researches as we do not yet have the dnb results of this session. 

For the second exploratory analysis, we wanted to compare the positive covid cases by regions. As population differs between departemnts, it was easier to compare using `incidence_rate`. To create the map, we selected from the covid_in_schools dataset the variables `department_code`, `incidence_rate`, `session`, `region`, `department_fr`  and `test_date`, did a group_by `region` and `test_date` and summarised `positive` by doing an average.
```{r , echo=TRUE, warning=FALSE, out.width='100%', out.height= '50%'}
p <- covid_in_schools %>%
...
##### 2020 {-}
*It is to be noted that for a better visual interpretation the scale is adapted for each graph.*
...
```
#### {.unnumbered}
During the 2020 session, the average incidence rate is relatively low in all departments of France, except in Mayenne where the incidence rate is 17.96, which is the highest rate. For the 2021 session, the scale indicates a clear increase in the proportions of average incidence rate in each of the departments, ranging from 246.9 to 52.6 cases per department. The western part of France is much more affected than the eastern part. We can hypothesise that this increase may be due to the fact that these are the departments closest to the borders and other countries. During the 2022 session, we can observe an overall increase in the incidence rate in each of the departments. Aveyron, Landes, Pyrénées-Atlantiques and Corse du Sud are the departments with the highest rates, with a maximum of 1201. For the 2023 session, the incidence rate fell significantly, particularly in Ile de France. This is because the school year is not over yet. La Creuse is the department with the highest rate with 420.
